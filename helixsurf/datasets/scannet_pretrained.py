...
from .utils import GtPretrainedRays, Intrin, Intrin_and_Inv, select_or_shuffle_rays


class ScanNetPretrainedDataset(DatasetBase):
    focal: float
    ...
    w: int
    n_images: int
    rays: Optional[GtPretrainedRays]
    split: str
    ...
    def __init__(
        ...
        **kwargs,
        ...
        pretrained_dir = os.path.join(self.instance_dir, "pretrained")
        ...
        if split == "train":
            ...
            all_pretrained_normal = []
            all_pretrained_mask = []
            clean_suffix = "_clean"
            assert os.path.exists(mvs_root), f"mvs_root: {mvs_root} does not exist"
            assert os.path.exists(
                pretrained_dir
            ), f"pretrained_dir: {pretrained_dir} does not exist"
            print(f"Using pretrained dir : {pretrained_dir}")
        ...
        for idx, imgname in enumerate(
            ...
            if split == "train":
                ...
                pretrained_normal = np.load(
                    os.path.join(pretrained_dir, f"{imgname[:-4]}_normal.npy")
                )
                rot_mat = c2w[:3, :3]
                pretrained_mask = ~(pretrained_normal == -1).all(-1)
                pretrained_normal = pretrained_normal @ (rot_mat.T)
                all_pretrained_normal.append(torch.from_numpy(pretrained_normal))
                all_pretrained_mask.append(torch.from_numpy(pretrained_mask))
        ...
        if split == "train":
            ...
            self.pretrained_normal = torch.stack(all_pretrained_normal)
            self.pretrained_mask = torch.stack(all_pretrained_mask)
    ...
    def gen_rays_scannet_neus(self, factor: int = 1) -> None:
        ...
        self.rays_init = GtPretrainedRays(
            origins=origins,
            dirs=dirs,
            gt=gt,
            depth_gt=depth_gt,
            normal_gt=self.normal_gt.view(-1, 3) if self.input_normal else None,
            pretrained_normal=self.pretrained_normal.view(-1, 3),
            pretrained_mask=self.pretrained_mask.view(-1),
